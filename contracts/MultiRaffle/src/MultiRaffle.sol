...
contract MultiRaffle is Ownable, ERC721, VRFConsumerBase {
  ...
  function claimRaffle(uint256[] calldata tickets) external {
    ...
    for (uint256 i = 0; i < tickets.length; i++) {
      ...
      require(
        tickets[i] < raffleEntries.length,
        "Ticket is out of entries range"
      );
  ...
  function withdrawRaffleProceeds() external onlyOwner {
    ...
    uint256 proceeds = MINT_COST *
      (
        raffleEntries.length > AVAILABLE_SUPPLY // Mint cost * available supply if many entries
          ? AVAILABLE_SUPPLY // Else, mint cost * raffle entries
    ...
    returns (string memory)
  {
    ...
    for (uint256 i = 0; i < metadatas.length; i++) {
      if (
        tokenId >= metadatas[i].startIndex && tokenId < metadatas[i].endIndex
      ) {
    ...
    if (metadataCleared) {
      parts[1] = string(
        abi.encodePacked("Randomness: ", _toString(randomness))
      );
    } else {
