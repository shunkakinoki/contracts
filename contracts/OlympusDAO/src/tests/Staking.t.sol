...
import "../sOlympusERC20.sol";
import "../governance/gOHM.sol";
import "../Treasury.sol";
import "../StakingDistributor.sol";
import "../OlympusAuthority.sol";

import "./utils/Hevm.t.sol";
import "./utils/MockContract.t.sol";
...
contract StakingTest is DSTest {
  ...
  function setUp() public {
    ...
    ohm = new OlympusERC20(address(authority));
    ...
    treasury.enable(
      OlympusTreasury.STATUS.REWARDMANAGER,
      address(distributor),
      address(0)
    ); // Allows distributor to mint ohm.
    treasury.enable(
      OlympusTreasury.STATUS.RESERVETOKEN,
      address(mockToken),
      address(0)
    ); // Allow mock token to be deposited into treasury
    treasury.enable(
      OlympusTreasury.STATUS.RESERVEDEPOSITOR,
      address(this),
      address(0)
