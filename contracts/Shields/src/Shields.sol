...
contract Shields is ERC721, IShields, Ownable {
  ...
  modifier onlyTokenOwner(uint256 tokenId) {
    require(
      ...
      "Shields: only owner can build Shield"
    );
    _;
  }
  ...
  constructor(
    ...
    for (
      uint256 j = makerPremintThreshold + 1;
      j <= granteePremintThreshold;
      j++
    ) {
    ...
    validColors(field, colors)
  {
    ...
    // Calculate Fee
    {
      ...
      ICategories.HardwareCategories hardwareType = emblemWeaver
        ...
        .hardwareType;
      uint256 frameFee = emblemWeaver
        .frameGenerator()
        .generateFrame(shield.frame)
        .fee;
  ...
  function calculateShieldBadge(uint256 tokenId)
    internal
    pure
    returns (ShieldBadge)
  {
