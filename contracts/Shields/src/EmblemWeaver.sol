...
contract EmblemWeaver {
  ...
  {
    IFieldSVGs.FieldData memory field = fieldGenerator.generateField(
      shield.field,
      shield.colors
    );
    IHardwareSVGs.HardwareData memory hardware = hardwareGenerator
      .generateHardware(shield.hardware);
    IFrameSVGs.FrameData memory frame = frameGenerator.generateFrame(
      shield.frame
    );

    string memory name = generateTitle(
      field.title,
      hardware.title,
      frame.title,
      shield.colors
    );
    bytes memory attributes = generateAttributesJSON(
      field,
      hardware,
      frame,
      shield.colors
          ...
          Base64.encode(
            bytes(
              abi.encodePacked(
                ...
                Base64.encode(
                  bytes(
                    shieldBadgeSVGGenerator.generateShieldBadgeSVG(shieldBadge)
                  )
                ),
    ...
    returns (string memory)
  {
    ...
    if (colors[3] > 0) {
      colorTitle = abi.encodePacked(
        colorTitle,
        " and ",
        fieldGenerator.colorTitle(colors[3])
      );
    }
  ...
  {
    if (bytes(frameTitle).length > 0) {
      frameAttribute = abi.encodePacked(
