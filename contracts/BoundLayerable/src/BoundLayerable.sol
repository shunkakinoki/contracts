...
abstract contract BoundLayerable is RandomTraits, BoundLayerableEvents {
  ...
  modifier canMint(uint256 numSets) {
    ...
    if (numSets * uint256(NUM_TOKENS_PER_SET) + _nextTokenId() > MAX_TOKEN_ID) {
      revert MaxSupply();
    }
    _;
  }
  ...
  function _burnAndBindMultiple(
    ...
    bytes32 baseSeed = getRandomnessForTokenIdFromSeed(baseTokenId, traitSeed);
    ...
    unchecked {
      ...
      for (; i < length; ) {
        ...
        bytes32 layerSeed = getRandomnessForTokenIdFromSeed(tokenId, traitSeed);
    ...
    virtual
  {
    // check ownership
    if (
      ownerOf(baseTokenId) != msg.sender || ownerOf(layerTokenId) != msg.sender
    ) {
      revert NotOwner();
    }
   ...
   */
  function _unpackLayersToBitMapAndCheckForDuplicates(uint256 bytePackedLayers)
    internal
    virtual
    returns (uint256 bitMap, uint256 numLayers)
  {
  ...
  function _setMetadataContract(ILayerable _metadataContract) internal virtual {
