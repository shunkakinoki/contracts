...
import { MAX_INT, _32_MASK, BATCH_NOT_REVEALED_SIGNATURE } from "../../interface/Constants.sol";
import { MaxRandomness, UnsafeReveal, BatchNotRevealed } from "../../interface/Errors.sol";
...
contract BatchVRFConsumerImpl is BatchVRFConsumer {
  ...
  function getRandomnessForBatchId(uint256 batchId)
    ...
    returns (bytes32)
  {
    return getRandomnessForTokenIdPub(batchId * NUM_TOKENS_PER_RANDOM_BATCH);
  }
...
contract BatchVRFConsumerTest is Test {
  ...
  function setUp() public {
    test = new BatchVRFConsumerImpl("test", "test", address(this), 8000, 7, 1);
  }
  ...
  function testRequestRandomWords(uint8 completedBatches, uint8 revealedBatches)
    public
  {
  ...
  function testFulfillRandomWords(uint8 length) public {
    ...
    for (uint8 i = 0; i < length; i++) {
      if (i > 4) {
        vm.expectRevert(abi.encodeWithSelector(BatchNotRevealed.selector));
      }
      unchecked {
        assertEq(
