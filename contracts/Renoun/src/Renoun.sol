...
contract Renoun is ERC721 {
  ...
  function mint(
    ...
    require(
      _pullRequestID > 0,
      "Renoun: Pull request ID must be greater than 0"
    );
  ...
  function changeRenderer(address _newRenderer) public returns (bool) {
    require(
      msg.sender == _admin,
      "Renoun: Only the admin can change the renderer address"
    );
    require(
      _newRenderer != address(0),
      "Renoun: Cannot change to the null address"
    );
  ...
  function _renderSVG(Contribution memory _contribution)
    internal
    view
    returns (string memory)
  {
