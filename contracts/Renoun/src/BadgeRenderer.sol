...
contract BadgeRenderer {
  ...
  function renderPullRequest(
    ...
    return
      string(
        abi.encodePacked(
          _buildPartOne(_contribution),
          _buildPartTwo(_contribution),
          _buildPartThree(_contribution)
        )
      );
  }
  ...
  function _formatInteger(uint256 _integerAsUint, uint256 _precision)
    internal
    pure
    returns (string memory)
  {
    ...
    return
      string(
        abi.encodePacked(
          _integerToString(_quotient),
          ".",
          _integerToString(_remainder),
          suffices[(len - 1) / 3]
        )
      );
  }
  ...
  function _buildPartOne(Contribution memory _contribution)
    internal
    pure
    returns (string memory)
  {
    ...
    return
      string(
        abi.encodePacked(
          ...
          "' style='margin-left: auto; border-radius: 50%; width: 25px; height: 25px; margin-right:7px;'></img> <span xmlns='http://www.w3.org/1999/xhtml' style='position: absolute; right: 0; max-width: 120px; overflow: hidden; transform: translateY(5px); font-size: 15px; font-weight: 450; font-family: arial; color: #ffffff;'>",
          _contribution._pullRequestCreatorUsername,
          "</span></foreignObject> <text x='177' y='293.5' fill='#bfbfbf' font-size='11.5px' font-weight='400' font-family='arial'>",
          _contribution._commitHash,
          "</text> <text x='130' y='342' fill='#808080' font-size='14px' font-weight='300' font-family='arial'>",
  ...
  function _buildPartThree(Contribution memory _contribution)
    internal
    pure
    returns (string memory)
  {
