...
contract TestProxy is DSTest {
  ...
  function testProxy(uint128 amt) public {
    assertEq(
      proxy1.balanceOf(address(user1)),
      proxy2.balanceOf(address(user1))
    );
    assertEq(
      proxy1.balanceOf(address(user2)),
      proxy2.balanceOf(address(user2))
    );

    uint256 preBal1 = proxy2.balanceOf(address(user1));
    uint256 preBal2 = proxy1.balanceOf(address(user2));
